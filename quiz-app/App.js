...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Image, ScrollView, SafeAreaView, LayoutAnimation, UIManager, Alert } from 'react-native';
...
const HomeScreen = ({ navigation }) => {
  ...
  const loadHearts = async () => {
    try {
      const storedHearts = await AsyncStorage.getItem('hearts');
      if (storedHearts !== null) {
        setHearts(parseInt(storedHearts));
      }
    } catch (error) {
  ...
  const navigateToQuestion = (subject) => {
    if (hearts > 0) {
      navigation.navigate('Question', { subject, initialHearts: hearts });
    } else {
      Alert.alert(
        "Out of Hearts",
        "Would you like to watch an ad to refill your hearts?",
        [
          {
            text: "No",
            style: "cancel"
          ...
          { 
            text: "Yes", 
            onPress: () => {
              // Here you would typically trigger your ad logic
              console.log("User agreed to watch an ad");
              updateHearts(5); // Refill hearts after ad
            }
          }
        ]
      );
  ...
  const updateHearts = async (newHeartCount) => {
    try {
      await AsyncStorage.setItem('hearts', newHeartCount.toString());
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        <Text style={styles.name}>The Kms</Text>
        ...
        <View style={styles.subjectsGrid}>
          <SubjectButton icon="cube-outline" label="Biology" onPress={() => navigateToQuestion('Biology')} />
          <SubjectButton icon="atom" label="Physics" onPress={() => navigateToQuestion('Physics')} />
          <SubjectButton icon="flask" label="Chemistry" onPress={() => navigateToQuestion('Chemistry')} />
          <SubjectButton icon="function" label="Maths" onPress={() => navigateToQuestion('Maths')} />
        </View>
...
const QuestionScreen = ({ route, navigation }) => {
  const { subject, initialHearts } = route.params;
  ...
  const handleOptionSelect = (index) => {
    ...
    if (index !== currentSubjectQuestions[currentQuestion].answer) {
      ...
      if (newHearts <= 0) {
        Alert.alert(
          ...
          [
            {
              text: "No",
              onPress: () => navigation.navigate('Home', { updatedHearts: 0 }),
              style: "cancel"
            },
            { 
              text: "Yes", 
              onPress: () => {
                ...
                updateHearts(refillAmount);
                navigation.navigate('Home', { updatedHearts: refillAmount });
              }
            }
          ]
        );
  ...
  const handleNextQuestion = () => {
    if (currentQuestion < currentSubjectQuestions.length - 1) {
      ...
      navigation.navigate('Home', { updatedHearts: hearts });
    }
  };
...
const styles = StyleSheet.create({
  ...
  progressGrid: {
    ...
    flexWrap: 'wrap',
    justifyContent: 'space-between',
    marginTop: 10, // Added top margin to create some space after the header
  },
  questionButton: {
    width: '18%', // Slightly increased width for better spacing
    aspectRatio: 1,
    borderRadius: 10,
    justifyContent: 'center',
    alignItems: 'center',
    marginBottom: 10,
    marginHorizontal: '1%', // Added horizontal margin for even spacing
  },
  completedQuestion: {
    backgroundColor: '#E1F396'
  },
  ...
  progressContainer: {
    marginTop: 30, // Add margin top to create space between header and progress bar
    marginBottom: 20, // Add margin bottom for space below progress bar
  },
  leaderboardHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
